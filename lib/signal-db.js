...
function getMessages(db, options = {}) {
  ...
  if (incoming && outgoing) {
    throw new Error('Nelze kombinovat --incoming a --outgoing');
  }
  if (unread && outgoing) {
    throw new Error('Nelze kombinovat --unread a --outgoing (nepřečtené jsou vždy příchozí)');
  }
  ...
  if (conv) {
    if (/^[0-9a-f]+(-[0-9a-f]+){3,}$/i.test(conv)) {
      ...
      if (convs.length === 0) {
        throw new Error(`Konverzace nenalezena: "${exact}"`);
      }
