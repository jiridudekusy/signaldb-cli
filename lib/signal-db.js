...
function getMessages(db, options = {}) {
  ...
  // rottingCount subselect for unanswered mode
  if (unanswered) {
    const cutoff = Date.now() - olderThan * 60 * 60 * 1000;
    selectCols.push(`(SELECT COUNT(*) FROM messages m3
  ...
  if (unanswered) {
    ...
    // Group by conversation: only the latest rotting message per conversation
    conditions.push(`m.sent_at = (
