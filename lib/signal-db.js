...
function formatMessage(msg, options = {}) {
  ...
  // In group chats the direction arrow alone doesn't say who wrote the message,
  // so surface the sender. Outgoing group messages are always from the local user.
  let sender = null;
  if (msg.conversationType === 'group') {
    sender = msg.type === 'outgoing'
      ? 'Me'
      : (msg.senderName || msg.senderPhone || '?');
  }
  return { body: `${body}${suffix}`, conv, dir, sender };
}
...
function getMessages(db, options = {}) {
  ...
  const selectCols = [
    ...
    'COALESCE(s.name, s.profileFullName, s.profileName, s.e164) AS senderName', 'm.source AS senderPhone',
  ];
...
const CONTEXT_SELECT = `
  SELECT m.id, m.body, m.sent_at, m.type, m.conversationId,
         COALESCE(c.name, c.profileFullName, c.profileName) AS conversationName,
         c.e164 AS conversationPhone,
         c.type AS conversationType,
         COALESCE(s.name, s.profileFullName, s.profileName, s.e164) AS senderName,
         m.source AS senderPhone
  FROM messages m
  LEFT JOIN conversations c ON m.conversationId = c.id
  LEFT JOIN conversations s ON m.sourceServiceId = s.serviceId
`;

function fetchContext(db, conversationId, anchorSentAt, before, after) {
  const beforeMsgs = before > 0
    ? db.prepare(`${CONTEXT_SELECT}
        WHERE m.conversationId = ? AND m.sent_at < ?
          AND m.body IS NOT NULL AND m.body != ''
          AND m.type IN ('incoming', 'outgoing')
        ORDER BY m.sent_at DESC LIMIT ?`).all([conversationId, anchorSentAt, before]).reverse()
    : [];
  const afterMsgs = after > 0
    ? db.prepare(`${CONTEXT_SELECT}
        WHERE m.conversationId = ? AND m.sent_at > ?
          AND m.body IS NOT NULL AND m.body != ''
          AND m.type IN ('incoming', 'outgoing')
        ORDER BY m.sent_at ASC LIMIT ?`).all([conversationId, anchorSentAt, after])
    : [];
  return { before: beforeMsgs, after: afterMsgs };
}

/**
 * Fetch messages matching a search query with surrounding context.
 * Returns { groups, total, conversationName? }.
 */
function getMessagesWithContext(db, options = {}) {
  const { before = 0, after = 0, ...msgOptions } = options;
  const result = getMessages(db, msgOptions);

  const contextMap = new Map();
  for (const match of result.messages) {
    contextMap.set(match.id, fetchContext(db, match.conversationId, match.sent_at, before, after));
  }

  const groups = mergeContextGroups(result.messages, contextMap);
  return { groups, total: result.total, conversationName: result.conversationName };
}

/**
 * Merge per-match context windows into non-overlapping groups.
 *
 * @param {object[]} matches – matched messages from getMessages()
 * @param {Map<string, { before: object[], after: object[] }>} contextMap – keyed by match id
 * @returns {object[]} groups – [{ conversationName, messages: [{ ...msg, isMatch }] }]
 */
function mergeContextGroups(matches, contextMap) {
  const byConv = new Map();
  for (const m of matches) {
    if (!byConv.has(m.conversationId)) byConv.set(m.conversationId, []);
    byConv.get(m.conversationId).push(m);
  }
  for (const arr of byConv.values()) arr.sort((a, b) => a.sent_at - b.sent_at);

  const groups = [];
  for (const [, convMatches] of byConv) {
    const windows = convMatches.map((match) => {
      const ctx = contextMap.get(match.id) || { before: [], after: [] };
      const msgs = new Map();
      for (const m of ctx.before) msgs.set(m.id, { ...m, isMatch: false });
      msgs.set(match.id, { ...match, isMatch: true });
      for (const m of ctx.after) if (!msgs.has(m.id)) msgs.set(m.id, { ...m, isMatch: false });
      return [...msgs.values()].sort((a, b) => a.sent_at - b.sent_at);
    });

    let current = windows[0];
    for (let i = 1; i < windows.length; i++) {
      const next = windows[i];
      const currentLast = current[current.length - 1].sent_at;
      const nextFirst = next[0].sent_at;
      if (nextFirst <= currentLast) {
        const merged = new Map();
        for (const m of current) merged.set(m.id, m);
        for (const m of next) if (!merged.has(m.id)) merged.set(m.id, m);
        for (const m of next) if (m.isMatch) merged.get(m.id).isMatch = true;
        current = [...merged.values()].sort((a, b) => a.sent_at - b.sent_at);
      } else {
        const convName = current[0].conversationName || current[0].conversationPhone || current[0].conversationId;
        groups.push({ conversationName: convName, messages: current });
        current = next;
      }
    }
    const convName = current[0].conversationName || current[0].conversationPhone || current[0].conversationId;
